...
float diffWAVE = 0;
int currentADCWAVE = 0;
float smoothADCWAVE = 0;
...
void loop() {
  ...
  currentADCWAVE = analogRead(A3);
  diffWAVE = currentADCWAVE - smoothADCWAVE;
  smoothADCWAVE = smoothADCWAVE + (diffWAVE * 0.01); // Decimal value is digital lowpass, lower value = more slew

  waveSelect = map(smoothADCWAVE, 1.0, 1023.0, 0, 90);
  ...
  smoothADCOSC = smoothADCOSC + (diffOSC * 0.01); // Decimal value is digital lowpass, lower value = more slew
  osc1 = mapfloat(smoothADCOSC, 0, 1023, 1.0, 500.0);
  ...
  // Filter 1
  cutoff1 = mapfloat(analogRead(A5), 0, 1023, 80.0, 15000.0);
  ...
  expPedal = map(smoothADCPEDAL, 1, 930, -512, 512);
